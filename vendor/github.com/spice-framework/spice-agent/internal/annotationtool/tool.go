...
// The compiler communicates with it only through the public v1alpha2 protocol.
...
const (
	...
	descriptorPackage  = modulePath + "/annotation/agent"
	providerCapability = string(sdk.ContributionProvider)
	metadataCapability = string(sdk.ContributionBeanMetadata)
)

// Tool is an isolated deterministic protocol implementation. Its handler map
// is protocol dispatch state, not an application extension graph or DI registry.
type Tool struct {
...
// New returns one isolated tool with statically declared typed handlers.
func New() *Tool {
...
func (tool *Tool) Initialize(ctx context.Context, params protocol.InitializeParams) (protocol.InitializeResult, error) {
	if tool == nil {
		return protocol.InitializeResult{}, errors.New("spice agent annotation tool is nil")
	}
	if err := contextError(ctx); err != nil {
		return protocol.InitializeResult{}, err
	}
	if params.Protocol != sdk.ProtocolV1Alpha2 {
		return protocol.InitializeResult{}, errors.New("spice agent annotation tool requires protocol " + string(sdk.ProtocolV1Alpha2))
	}
	if params.ToolPath != agenttool.Path {
		return protocol.InitializeResult{}, errors.New("spice agent annotation tool path identity does not match")
	}
...
// Describe returns stable sorted descriptor and contribution capabilities.
func (tool *Tool) Describe(ctx context.Context, _ protocol.DescribeParams) (protocol.DescribeResult, error) {
	if tool == nil {
		return protocol.DescribeResult{}, errors.New("spice agent annotation tool is nil")
	}
	if err := contextError(ctx); err != nil {
	...
	return protocol.DescribeResult{
		DescriptorPackages: []string{descriptorPackage},
		Handlers:           handlers,
	}, nil
}

// Analyze dispatches only the descriptor identities returned by Describe.
func (tool *Tool) Analyze(ctx context.Context, params protocol.AnalyzeParams) (protocol.AnalyzeResult, error) {
	if tool == nil {
		return protocol.AnalyzeResult{}, errors.New("spice agent annotation tool is nil")
	}
	if err := contextError(ctx); err != nil {
	...
	if params.Descriptor.Package != params.Invocation.DescriptorPackage ||
		...
		return protocol.AnalyzeResult{}, errors.New("spice agent annotation descriptor dispatch does not match invocation")
	}
	handler, found := tool.handlers[symbolKey(params.Descriptor)]
	if !found {
		return protocol.AnalyzeResult{}, errors.New("spice agent annotation handler is not declared")
	}
...
func (tool *Tool) Shutdown(ctx context.Context, _ protocol.ShutdownParams) error {
	if tool == nil {
		return errors.New("spice agent annotation tool is nil")
	}
...
func contextError(ctx context.Context) error {
	if ctx == nil {
		return errors.New("spice agent annotation tool context must not be nil")
	}
	return ctx.Err()
}
...
func handlerRegistrations() []handlerRegistration {
	return []handlerRegistration{
		newHandlerRegistration("Stage", agentannotation.Stage),
		newHandlerRegistration("Tool", agentannotation.Tool),
		newHandlerRegistration("ModelProvider", agentannotation.ModelProvider),
	}
}
...
func encodeHandlerResult(value sdk.Result) (protocol.AnalyzeResult, error) {
	result := protocol.AnalyzeResult{
		...
		Diagnostics:   append([]protocol.Diagnostic(nil), value.Diagnostics...),
	}
